...
function sys_tools_menu() {
    while true; do
        ...
        echo -e "${CYAN}================================================================${NC}"
        echo -e " ${GREEN}1.${NC}  高级防火墙管理                    ${GREEN}11.${NC} SSL 证书管理 (acme.sh)"
        echo -e " ${GREEN}2.${NC}  修改登录密码                      ${GREEN}12.${NC} 磁盘空间分析"
        echo -e " ${GREEN}3.${NC}  修改SSH连接端口                   ${GREEN}13.${NC} BTOP系统监控工具"
        echo -e " ${GREEN}4.${NC}  切换优先IPV4/IPV6                 ${GREEN}14.${NC} TCP窗口调优"
        echo -e " ${GREEN}5.${NC}  修改主机名                        ${GREEN}15.${NC} 进程管理工具（查看/终止）"
        echo -e " ${GREEN}6.${NC}  系统时区调整                      ${GREEN}16.${NC} 系统环境修复 (权限/磁盘/APT)"
        echo -e " ${GREEN}7.${NC}  修改虚拟内存大小(Swap)            ${GREEN}17.${NC} 配置中文语言支持 (Debian/Ubuntu)"
