...
function app_center_menu() {
    while true; do
        ...
        local c29=$(check_installed "docker" "portainer" && echo "$GREEN" || echo "$NC")
        local c30=$(check_installed "docker" "dockge" && echo "$GREEN" || echo "$NC")
        local c31=$(check_installed "docker" "pairdrop" && echo "$GREEN" || echo "$NC")
        ...
        echo -e " ${GREEN}29.${NC} ${c29}Portainer (Docker管理面板)${NC}      ${GREEN}30.${NC} ${c30}Dockge (Compose管理面板)${NC}"
        echo -e " ${GREEN}31.${NC} ${c31}PairDrop (局域网文件传输)${NC}"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
        echo -e " ${RED}0.${NC}  返回主菜单"
        echo -e "${CYAN}================================================================${NC}"
        read -p "请输入你的选择 (0-31) : " app_choice
        ...
        case "$app_choice" in
            ...
            29) portainer_management ;;
            30) dockge_management ;;
            31) pairdrop_management ;;
            0) break ;;
...
function portainer_management() {
    while true; do
        clear
        echo -e "${CYAN}=========================================${NC}"
        echo -e "${GREEN}           Portainer 管理${NC}"
        ...
        if docker ps -a --format '{{.Names}}' | grep -q "^portainer$"; then
            if docker inspect --format='{{.State.Status}}' portainer 2>/dev/null | grep -q "running"; then
                status_text="${GREEN}运行中${NC}"
                host_port=$(docker inspect portainer --format='{{(index (index .NetworkSettings.Ports "9000/tcp") 0).HostPort}}' 2>/dev/null)
            else
                status_text="${YELLOW}已停止${NC}"
            fi
        fi
        ...
        echo -e "${CYAN}=========================================${NC}"
        echo -e "Portainer 是轻量级 Docker 可视化管理面板，"
        echo -e "支持容器、镜像、网络、卷的图形化管理。"
        echo ""
        echo -e " ${GREEN}1.${NC} 安装/更新 Portainer"
        echo -e " ${GREEN}2.${NC} 启动 Portainer"
        echo -e " ${GREEN}3.${NC} 停止 Portainer"
        echo -e " ${GREEN}4.${NC} 重启 Portainer"
        echo -e " ${GREEN}5.${NC} 查看容器日志"
        echo -e " ${GREEN}6.${NC} 卸载 Portainer"
        echo -e "${CYAN}-----------------------------------------${NC}"
        echo -e " ${RED}0.${NC} 返回上一级菜单"
        echo -e "${CYAN}=========================================${NC}"
        read -p "请输入你的选择 (0-6): " port_choice

        case "$port_choice" in
            1) install_portainer ;;
            2) manage_portainer_container "start" ;;
            3) manage_portainer_container "stop" ;;
            4) manage_portainer_container "restart" ;;
            5) docker logs --tail 50 portainer; read -p "按回车键继续..." ;;
...
function install_portainer() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${GREEN}          安装/更新 Portainer${NC}"
    echo -e "${CYAN}=========================================${NC}"
    ...
    local deploy_dir="/opt/portainer"
    mkdir -p "$deploy_dir/data"

    local default_port=9000
    if [ -f "$deploy_dir/docker-compose.yml" ]; then
        local exist_port=$(grep -oP '^\s+-\s+"\K[0-9]+(?=:9000")' "$deploy_dir/docker-compose.yml" | head -1)
        default_port=${exist_port:-9000}
    fi

    read -p "请输入 Web UI 端口 (默认 $default_port): " host_port
    host_port=${host_port:-$default_port}
...
EOF
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Portainer 安装/更新成功！${NC}"
        IFS='|' read -r ipv4 ipv6 <<< "$(get_access_ips)"
        [ -n "$ipv4" ] && echo -e "访问地址: ${YELLOW}http://${ipv4}:${host_port}${NC}"
...
function manage_portainer_container() {
    local action=$1
    local deploy_dir="/opt/portainer"
    if [ -d "$deploy_dir" ] && [ -f "$deploy_dir/docker-compose.yml" ]; then
        cd "$deploy_dir"
        echo -e "${BLUE}正在对 Portainer 执行 ${action} 操作...${NC}"
        if docker compose version &> /dev/null; then
            docker compose "$action"
        else
            docker-compose "$action"
...
function uninstall_portainer() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${RED}           卸载 Portainer${NC}"
    echo -e "${CYAN}=========================================${NC}"

    read -p "确定要彻底删除 Portainer 吗？(y/N): " confirm
    if [[ "$confirm" =~ ^[yY]$ ]]; then
        local deploy_dir="/opt/portainer"
        if [ -d "$deploy_dir" ]; then
            cd "$deploy_dir"
            ...
            read -p "是否同步删除 Portainer 数据？(y/N): " del_data
            if [[ "$del_data" =~ ^[yY]$ ]]; then
                rm -rf "$deploy_dir"
                echo -e "${GREEN}✅ Portainer 已彻底卸载。${NC}"
            else
                rm -f "$deploy_dir/docker-compose.yml"
                echo -e "${YELLOW}已保留数据目录，仅移除容器。${NC}"
            fi
        else
            docker stop portainer &>/dev/null
            docker rm portainer &>/dev/null
...
function dockge_management() {
    while true; do
        clear
        echo -e "${CYAN}=========================================${NC}"
        echo -e "${GREEN}           Dockge 管理${NC}"

        local status_text="${RED}未安装${NC}"
        local host_port=""

        if docker ps -a --format '{{.Names}}' | grep -q "^dockge$"; then
            if docker inspect --format='{{.State.Status}}' dockge 2>/dev/null | grep -q "running"; then
                status_text="${GREEN}运行中${NC}"
                host_port=$(docker inspect dockge --format='{{(index (index .NetworkSettings.Ports "5001/tcp") 0).HostPort}}' 2>/dev/null)
            else
                status_text="${YELLOW}已停止${NC}"
            fi
        fi
        ...
        echo -e "${CYAN}=========================================${NC}"
        echo -e "Dockge 是美观易用的 Docker Compose 管理面板，"
        echo -e "支持 Compose 文件的编辑、启动、停止等操作。"
        echo ""
        echo -e " ${GREEN}1.${NC} 安装/更新 Dockge"
        echo -e " ${GREEN}2.${NC} 启动 Dockge"
        echo -e " ${GREEN}3.${NC} 停止 Dockge"
        echo -e " ${GREEN}4.${NC} 重启 Dockge"
        echo -e " ${GREEN}5.${NC} 查看容器日志"
        echo -e " ${GREEN}6.${NC} 卸载 Dockge"
        echo -e "${CYAN}-----------------------------------------${NC}"
        echo -e " ${RED}0.${NC} 返回上一级菜单"
        echo -e "${CYAN}=========================================${NC}"
        read -p "请输入你的选择 (0-6): " dockge_choice

        case "$dockge_choice" in
            1) install_dockge ;;
            2) manage_dockge_container "start" ;;
            3) manage_dockge_container "stop" ;;
            4) manage_dockge_container "restart" ;;
            5) docker logs --tail 50 dockge; read -p "按回车键继续..." ;;
            6) uninstall_dockge ;;
            0) break ;;
            *) echo -e "${RED}无效的选择，请重新输入！${NC}"; sleep 1 ;;
        esac
...
function install_dockge() {
    ...
    local deploy_dir="/opt/dockge"
    mkdir -p "$deploy_dir/data"
    mkdir -p "$deploy_dir/stacks"

    local default_port=5001
    if [ -f "$deploy_dir/docker-compose.yml" ]; then
        local exist_port=$(grep -oP '^\s+-\s+"\K[0-9]+(?=:5001")' "$deploy_dir/docker-compose.yml" | head -1)
        default_port=${exist_port:-5001}
    fi
...
EOF
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "^dockge$"; then
        docker stop dockge &>/dev/null
        docker rm dockge &>/dev/null
    fi
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Dockge 安装/更新成功！${NC}"
        IFS='|' read -r ipv4 ipv6 <<< "$(get_access_ips)"
        [ -n "$ipv4" ] && echo -e "访问地址: ${YELLOW}http://${ipv4}:${host_port}${NC}"
    else
        echo -e "${RED}❌ 部署失败。${NC}"
...
function manage_dockge_container() {
    local action=$1
    local deploy_dir="/opt/dockge"
    if [ -d "$deploy_dir" ] && [ -f "$deploy_dir/docker-compose.yml" ]; then
        cd "$deploy_dir"
        echo -e "${BLUE}正在对 Dockge 执行 ${action} 操作...${NC}"
        if docker compose version &> /dev/null; then
            docker compose "$action"
        else
            docker-compose "$action"
...
function uninstall_dockge() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${RED}           卸载 Dockge${NC}"
    echo -e "${CYAN}=========================================${NC}"

    read -p "确定要彻底删除 Dockge 吗？(y/N): " confirm
    if [[ "$confirm" =~ ^[yY]$ ]]; then
        local deploy_dir="/opt/dockge"
        if [ -d "$deploy_dir" ]; then
            cd "$deploy_dir"
            ...
            read -p "是否同步删除 Dockge 数据及 Stack 配置？(y/N): " del_data
            if [[ "$del_data" =~ ^[yY]$ ]]; then
                rm -rf "$deploy_dir"
                echo -e "${GREEN}✅ Dockge 已彻底卸载。${NC}"
            else
                rm -f "$deploy_dir/docker-compose.yml"
                echo -e "${YELLOW}已保留数据和 Stack 目录，仅移除容器。${NC}"
            fi
        else
            docker stop dockge &>/dev/null
            docker rm dockge &>/dev/null
...
# PairDrop 局域网文件传输工具
function pairdrop_management() {
    while true; do
        clear
        echo -e "${CYAN}=========================================${NC}"
        echo -e "${GREEN}           PairDrop 管理${NC}"
        ...
        echo -e "${CYAN}=========================================${NC}"
        echo -e "PairDrop 是一款开源的局域网/互联网文件传输"
        echo -e "工具，支持端到端加密，无需注册即开即用。"
        echo ""
        echo -e " ${GREEN}1.${NC} 安装/更新 PairDrop"
        echo -e " ${GREEN}2.${NC} 启动 PairDrop"
        echo -e " ${GREEN}3.${NC} 停止 PairDrop"
        echo -e " ${GREEN}4.${NC} 重启 PairDrop"
        echo -e " ${GREEN}5.${NC} 查看容器日志"
        echo -e " ${GREEN}6.${NC} 卸载 PairDrop"
        echo -e "${CYAN}-----------------------------------------${NC}"
        echo -e " ${RED}0.${NC} 返回上一级菜单"
        echo -e "${CYAN}=========================================${NC}"
        read -p "请输入你的选择 (0-6): " pair_choice

        case "$pair_choice" in
            1) install_pairdrop ;;
            2) manage_pairdrop_container "start" ;;
            3) manage_pairdrop_container "stop" ;;
            4) manage_pairdrop_container "restart" ;;
            5) docker logs --tail 50 pairdrop; read -p "按回车键继续..." ;;
            6) uninstall_pairdrop ;;
            0) break ;;
            *) echo -e "${RED}无效的选择，请重新输入！${NC}"; sleep 1 ;;
        esac
    done
...
function install_pairdrop() {
    ...
    local deploy_dir="/opt/pairdrop"
    mkdir -p "$deploy_dir"

    local default_port=3000
    if [ -f "$deploy_dir/docker-compose.yml" ]; then
        local exist_port=$(grep -oP '^\s+-\s+"\K[0-9]+(?=:3000")' "$deploy_dir/docker-compose.yml" | head -1)
        default_port=${exist_port:-3000}
    fi
...
EOF
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ PairDrop 安装/更新成功！${NC}"
        IFS='|' read -r ipv4 ipv6 <<< "$(get_access_ips)"
        [ -n "$ipv4" ] && echo -e "访问地址: ${YELLOW}http://${ipv4}:${host_port}${NC}"
    else
        echo -e "${RED}❌ 部署失败。${NC}"
...
function manage_pairdrop_container() {
    local action=$1
    local deploy_dir="/opt/pairdrop"
    if [ -d "$deploy_dir" ] && [ -f "$deploy_dir/docker-compose.yml" ]; then
        cd "$deploy_dir"
        echo -e "${BLUE}正在对 PairDrop 执行 ${action} 操作...${NC}"
        if docker compose version &> /dev/null; then
            docker compose "$action"
        else
            docker-compose "$action"
...
function uninstall_pairdrop() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${RED}           卸载 PairDrop${NC}"
    echo -e "${CYAN}=========================================${NC}"

    read -p "确定要彻底删除 PairDrop 吗？(y/N): " confirm
    if [[ "$confirm" =~ ^[yY]$ ]]; then
        local deploy_dir="/opt/pairdrop"
        if [ -d "$deploy_dir" ]; then
            cd "$deploy_dir"
            ...
            cd - > /dev/null
            rm -rf "$deploy_dir"
            echo -e "${GREEN}✅ PairDrop 已彻底卸载。${NC}"
        else
            docker stop pairdrop &>/dev/null
            docker rm pairdrop &>/dev/null
            echo -e "${GREEN}容器已移除。${NC}"
