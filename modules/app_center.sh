...
function app_center_menu() {
    while true; do
        ...
        echo -e " ${GREEN}27.${NC} Uptime Kuma (站点监控)          ${GREEN}28.${NC} KVideo (视频聚合平台)"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
...
# KVideo 视频聚合平台管理模块
function kvideo_management() {
    while true; do
        clear
        echo -e "${CYAN}=========================================${NC}"
        echo -e "${GREEN}             KVideo 视频聚合平台${NC}"
        
        # 状态检测逻辑
        local status_text="${RED}未安装${NC}"
        local host_port=""
        
        if docker ps -a --format '{{.Names}}' | grep -q "^kvideo$"; then
            if docker inspect --format='{{.State.Status}}' kvideo 2>/dev/null | grep -q "running"; then
                status_text="${GREEN}运行中${NC}"
                host_port=$(docker inspect kvideo --format='{{(index (index .NetworkSettings.Ports "3000/tcp") 0).HostPort}}' 2>/dev/null)
            else
                status_text="${YELLOW}已停止${NC}"
            fi
        fi
        ...
        echo -e "${CYAN}=========================================${NC}"
        echo "KVideo 是一个现代化视频聚合播放平台"
        echo "基于 Next.js 16 构建，采用 Liquid Glass 设计语言"
        echo ""
        echo -e " ${GREEN}1.${NC} 安装/更新 KVideo"
        echo -e " ${GREEN}2.${NC} 启动 KVideo"
        echo -e " ${GREEN}3.${NC} 停止 KVideo"
        echo -e " ${GREEN}4.${NC} 重启 KVideo"
        echo -e " ${GREEN}5.${NC} 查看容器日志"
        echo -e " ${GREEN}6.${NC} 修改配置"
        echo -e " ${GREEN}7.${NC} 卸载 KVideo"
        echo -e " ${GREEN}8.${NC} 访问 Web 界面"
        echo -e "${CYAN}-----------------------------------------${NC}"
        echo -e " ${RED}0.${NC} 返回上一级菜单"
        echo -e "${CYAN}=========================================${NC}"
        read -p "请输入你的选择 (0-8): " kvideo_choice

        case "$kvideo_choice" in
            1) install_kvideo ;;
            2) docker start kvideo && echo -e "${GREEN}已启动${NC}" || echo -e "${RED}启动失败${NC}"; sleep 1 ;;
            3) docker stop kvideo && echo -e "${GREEN}已停止${NC}" || echo -e "${RED}停止失败${NC}"; sleep 1 ;;
            4) docker restart kvideo && echo -e "${GREEN}已重启${NC}" || echo -e "${RED}重启失败${NC}"; sleep 1 ;;
            5) docker logs --tail 50 kvideo; read -p "按回车键继续..." ;;
            6) modify_kvideo_config ;;
            7) uninstall_kvideo ;;
            8) access_kvideo_web ;;
            0) break ;;
            *) echo -e "${RED}无效的选择，请重新输入！${NC}"; sleep 1 ;;
...
function install_kvideo() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${GREEN}          安装/更新 KVideo${NC}"
    echo -e "${CYAN}=========================================${NC}"
    ...
    local deploy_dir="/opt/kvideo"
    mkdir -p "$deploy_dir"

    if docker ps -a --format '{{.Names}}' | grep -q "^kvideo$"; then
        echo -e "${YELLOW}检测到 KVideo 已安装。${NC}"
        read -p "是否重新安装？(y/N): " reinstall
        [[ ! "$reinstall" =~ ^[yY]$ ]] && return
    ...
    read -p "请输入管理员密码 (ADMIN_PASSWORD): " admin_pwd
    read -p "请输入访问密码 (ACCESS_PASSWORD, 可选): " access_pwd
    read -p "请输入站点名称 (默认 KVideo): " site_name
    site_name=${site_name:-"KVideo"}

    echo -e "${BLUE}正在部署 KVideo 容器...${NC}"
...
function modify_kvideo_config() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${GREEN}           修改 KVideo 配置${NC}"
    echo -e "${CYAN}=========================================${NC}"

    local deploy_dir="/opt/kvideo"
    if [ ! -f "$deploy_dir/.env" ]; then
        echo -e "${RED}未检测到安装配置信息。${NC}"
        read -p "按回车键继续..."
        return
    fi
    ...
    echo -e "访问密码: ${GREEN}${ACCESS_PASSWORD:-无}${NC}"
    echo -e "站点名称: ${GREEN}${SITE_NAME}${NC}"
    echo ""

    read -p "请输入新的端口 [${HOST_PORT}]: " new_port
    new_port=${new_port:-$HOST_PORT}
    
    read -p "请输入新的管理员密码 [${ADMIN_PASSWORD}]: " new_admin_pwd
    new_admin_pwd=${new_admin_pwd:-$ADMIN_PASSWORD}

    read -p "请输入新的访问密码 [${ACCESS_PASSWORD}]: " new_access_pwd
    new_access_pwd=${new_access_pwd:-$ACCESS_PASSWORD}

    read -p "请输入新的站点名称 [${SITE_NAME}]: " new_site_name
...
ACCESS_PASSWORD=${new_access_pwd}
...
function uninstall_kvideo() {
    read -p "确定要卸载 KVideo 吗？(y/N): " confirm
    if [[ "$confirm" =~ ^[yY]$ ]]; then
        docker stop kvideo &>/dev/null
        docker rm kvideo &>/dev/null
...
function access_kvideo_web() {
    clear
    echo -e "${CYAN}=========================================${NC}"
    echo -e "${GREEN}        访问 KVideo Web 界面${NC}"
    echo -e "${CYAN}=========================================${NC}"
    
    if ! docker ps -a --format '{{.Names}}' | grep -q "^kvideo$"; then
        echo -e "${RED}❌ KVideo 容器不存在。${NC}"
        return
    fi
    
    local host_port=$(docker inspect kvideo --format='{{(index (index .NetworkSettings.Ports "3000/tcp") 0).HostPort}}' 2>/dev/null)
    IFS='|' read -r ipv4 ipv6 <<< "$(get_access_ips)"
    
    echo -e "访问地址："
    [ -n "$ipv4" ] && echo -e "IPv4: ${YELLOW}http://${ipv4}:${host_port}${NC}"
    [ -n "$ipv6" ] && echo -e "IPv6: ${YELLOW}http://[${ipv6}]:${host_port}${NC}"
    
    if [ -f "/opt/kvideo/.env" ]; then
        source "/opt/kvideo/.env"
        echo ""
        echo -e "管理员密码: ${GREEN}${ADMIN_PASSWORD}${NC}"
        [ -n "$ACCESS_PASSWORD" ] && echo -e "访问密码: ${GREEN}${ACCESS_PASSWORD}${NC}"
    fi
    echo -e "${CYAN}=========================================${NC}"
    read -p "按回车键返回..."
}
